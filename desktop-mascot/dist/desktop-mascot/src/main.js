 ...
 */
const extensionServerPort = parsePortFromArgv(process.argv);
/**
 ...
 */
function postActionToExtension(port, action) {
    ...
    req.on('error', (err) => {
        console.error(`[Pluvianidae Mascot] Error al reenviar MascotAction a la extensión (POST ${MASCOT_ACTIONS_PATH}): ${err.message}`);
    });
    req.write(body);
    req.end();
}
 ...
 * sitios.
 */
function processMascotAction(action) {
    ...
    if (extensionServerPort === undefined) {
        console.error('[Pluvianidae Mascot] No se pudo reenviar la acción: el puerto del servidor de la extensión no se pudo ' +
 ...
 */
function applyLocalActionEffect(action) {
 ...
 */
function registerMascotActionChannel() {
    electron_1.ipcMain.on(ipc_channels_1.MASCOT_ACTION_CHANNEL, (_event, data) => {
        if (!(0, mascot_actions_1.isMascotAction)(data)) {
...
function computeInitialPosition() {
    const savedPosition = (0, window_state_1.readSavedPosition)((0, window_state_1.getWindowPositionFilePath)(electron_1.app.getPath('userData')));
    return (0, window_state_1.resolveInitialPosition)(savedPosition, electron_1.screen.getPrimaryDisplay().workArea, electron_1.screen.getAllDisplays().map((display) => display.workArea));
}
/** Crea y muestra la ventana flotante de la mascota. */
function createMascotWindow() {
    const initialPosition = computeInitialPosition();
    const win = new electron_1.BrowserWindow({
        width: window_state_1.WINDOW_WIDTH,
        height: window_state_1.WINDOW_HEIGHT,
        x: initialPosition.x,
        y: initialPosition.y,
        frame: false,
        transparent: true,
    ...
    // persistir el tamaño.
    const positionSaver = (0, window_state_1.createDebouncedPositionSaver)((0, window_state_1.getWindowPositionFilePath)(electron_1.app.getPath('userData')));
    win.on('moved', () => {
    ...
    win.webContents.on('context-menu', (_event) => {
        const menu = (0, context_menu_1.buildMascotContextMenu)(processMascotAction);
