...
/**
 * Los 7 estados visuales soportados por `styles.css` (una clase
 * `mascot-state-<estado>` por cada uno). No es un subconjunto de
 * `MascotEvent['type']`: existe una relación muchos-a-uno (varios tipos de
 * evento pueden mapear al mismo estado visual, p. ej. `seed` y `success`
 * comparten `success`), y `thinking` no tiene ningún evento que lo
 * dispare todavía (ver doc comment de este módulo).
 */
export type MascotVisualState = 'idle' | 'thinking' | 'working' | 'success' | 'warning' | 'error' | 'sleeping';

/** Prefijo común de las clases CSS de estado, usado tanto aquí como en `styles.css`. */
export const MASCOT_STATE_CLASS_PREFIX = 'mascot-state-';

/** Todos los estados visuales soportados, usado para poder removerlos todos antes de aplicar uno nuevo. */
export const MASCOT_VISUAL_STATES: readonly MascotVisualState[] = [
  'idle',
  'thinking',
  'working',
  'success',
 ...
 * un `type` desconocido.
 */
export function mapEventToVisualState(event: MascotEvent): MascotVisualState {
  switch (event.type) {
    case 'idle':
      return 'idle';
    case 'indexing':
 ...
 */
export function applyVisualState(root: HTMLElement, state: MascotVisualState): void {
 ...
 */
export const BUBBLE_AUTO_HIDE_MS = 4000;

/** Clase CSS (ver `styles.css`) que hace visible `#mascot-bubble`. */
export const BUBBLE_VISIBLE_CLASS = 'mascot-bubble-visible';
 ...
 */
export function extractBubbleText(event: MascotEvent): string | undefined {
  switch (event.type) {
    case 'success':
    case 'warning':
    case 'error':
 ...
 */
export function showBubble(bubbleElement: HTMLElement, text: string, autoHideMs: number = BUBBLE_AUTO_HIDE_MS): void {
 ...
 * `window.pluvianidae`.
 */
function initMascotRenderer(): void {
