...
/**
 * Every message ever sent from the desktop mascot to the extension.
 * Kept as a small, closed, discriminated union — mirrors the same
 * discipline as `MascotEvent` in `shared/mascot-events.ts` — so both ends
 * can validate incoming messages exhaustively (see `isMascotAction`
 * below) rather than trusting arbitrary JSON.
 */
